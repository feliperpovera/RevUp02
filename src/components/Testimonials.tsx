...
export const Testimonials = () => {
  return (
    <section id="testimonials" className="py-16 md:py-24 bg-background relative overflow-hidden" aria-labelledby="testimonials-heading">
      <CirclesDecor className="absolute top-10 right-20 opacity-20 animate-pulse" aria-hidden="true" />
      <AsteriskDecor className="absolute bottom-10 left-20 opacity-20" aria-hidden="true" />
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        <header className="text-center mb-10 md:mb-16">
          <h2 id="testimonials-heading" className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold mb-3 md:mb-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 max-w-5xl mx-auto">
          {testimonials.map((testimonial, index) => (
            ...
            >
              <Quote className="w-10 h-10 md:w-12 md:h-12 text-accent/20 absolute top-4 md:top-6 right-4 md:right-6" aria-hidden="true" />
              
              <p itemProp="reviewBody" className="text-base md:text-lg text-foreground/80 leading-relaxed mb-4 md:mb-6 relative z-10 pr-8">
                &quot;{testimonial.quote}&quot;
              </p>
              
              <div className="border-t border-border/30 pt-3 md:pt-4" itemProp="author" itemScope itemType="https://schema.org/Person">
                <div className="font-semibold text-foreground text-sm md:text-base" itemProp="name">{testimonial.author}</div>
                <div className="text-xs md:text-sm text-accent" itemProp="jobTitle">{testimonial.role}</div>
