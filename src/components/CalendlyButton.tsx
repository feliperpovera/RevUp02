...
const DEFAULT_LABEL = { en: "Book a Meeting on Calendly", es: "Agenda una reunión en Calendly" };

/** Calendly brand mark (Simple Icons, CC0) and brand blue #006BFF. */
export const CalendlyButton = ({ label, className }: { label?: string; className?: string }) => {
  const lang = useLang();
  return (
    <a
      href={CALENDLY_URL}
      ...
      className={cn(
        "inline-flex h-12 items-center justify-center gap-2.5 rounded-full bg-[#006BFF] px-6 text-sm font-semibold text-white transition-colors hover:bg-[#0058d6] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#006BFF] focus-visible:ring-offset-2",
        className
      )}
    >
