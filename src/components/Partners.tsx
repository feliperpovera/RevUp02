      ...
      <div className="container mx-auto px-4 md:px-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8 items-center justify-items-center max-w-5xl mx-auto">
          {partners.map((partner) => (
            <div
              key={partner.name}
              className="glass-card p-6 md:p-8 rounded-xl md:rounded-2xl hover:scale-105 transition-transform duration-300 w-full flex items-center justify-center"
            >
              <span className="text-xl md:text-2xl font-heading font-bold text-foreground/80 hover:text-accent transition-colors">
                {partner.name}
              </span>
            </div>
          ))}
